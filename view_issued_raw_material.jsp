...
<head>
    ...
    <title>View Issued Raw Materials</title>
...
<body>
    ...
    <main class="main-content">
        <h2>View Issued Raw Materials</h2>
        <table class="table table-striped">
            ...
            <tbody>
                <% 
                    List<IssueRaw> issuedRawMaterials = (List<IssueRaw>) request.getAttribute("issuedRawMaterials");
                    if (issuedRawMaterials != null && !issuedRawMaterials.isEmpty()) {
                        int serialNo = 1; // Initialize serial number
                        for (IssueRaw issueRaw : issuedRawMaterials) { 
                %>
                        <tr>
                            <td><%= serialNo++ %></td> <!-- Display serial number -->
                            <td><%= issueRaw.getIssuerName() %></td>
                            <td><%= issueRaw.getRawName() %></td>
                            <td><%= issueRaw.getQuantity() %></td>
                            <td><%= issueRaw.getIssueDate() %></td>
                            <td><%= issueRaw.getIssueTime() %></td>
                        </tr>
                <% 
                        } 
                    } else { 
                %>
                    <tr>
                        <td colspan="6">No issued raw materials found.</td>
                    </tr>
                <% 
                    } 
                %>
